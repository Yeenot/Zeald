...
class Display
{
    public static function format($data, $format = 'html') {
        ...
        switch($format) {
            ...
            default: // html
                if (!$data->count()) {
                    return self::htmlTemplate('Sorry, no matching data was found');
                }
                ...
                $rows = implode('', $rows);
                return self::htmlTemplate('<table>' . $headings . $rows . '</table>');
                break;
        }
    }

    // wrap html in a standard template
    private static function htmlTemplate($html) {
        return '
        <html>
        <head>
        <style type="text/css">
